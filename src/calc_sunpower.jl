...
const SUN_AREA = 16.0 # area of the solar panels in m²

function calc_average_power(df)
    return SUN_AREA * mean(df.Power_Generation)
end

function calc_peak_power(df)
    return SUN_AREA * maximum(df.Power_Generation)
end
...
function calc_rel_availability(df, min_power)
    minpower = df[df.Power_Generation * SUN_AREA .> min_power, :Power_Generation]
...
println("Percentage of solar power availability   [%]: ", round(calc_rel_availability(df, 2.0), digits=1))
